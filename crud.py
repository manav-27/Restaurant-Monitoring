...
def make_report(db: Session,report_id):
        ...
        store_working_hours = Store_Working_Hours.order_by(StoreWorkingHours.store_id).all()[-11:-1]
        timezones = timeZone.order_by(Timezones.store_id).all()[-11:-1]
        store_records = Store_Records.order_by(Storerecords.store_id).all()[-11:-1]
        ...
        for store in Map_Of_StoreID_With_Timestamps:
            flag=0;
            Map_Of_answer_for_each_store[store] = [0,3,4,0,0,0]
